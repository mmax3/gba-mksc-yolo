            ...
            for stmt in msg.split("; "):
                ...
                #
                    ...
                    if var.startswith("screenshots "): response += self.screenshots[int(var[11:])].decode("utf-8")
                        ...
                        if self.mode == "HUMAN":
                            #pass
                            response += "" # human control, so no commands/response will be sent
                        else:
                            response += dict_as_str(self.controls)
                    # Strings
                ...
                #
                    ...
                    if after_set.startswith("screenshots"):
                        ...
                        thisdict = {}
                        splt2 = after_set.split(',')[1:]
                        for item in splt2:
                            key,val = item.split(':')
                            #thisdict[key] = val.capitalize()
                            thisdict[key] = eval(val.capitalize())
                            self.controls = thisdict.copy()
                        #print(self.controls)
                    # Strings
