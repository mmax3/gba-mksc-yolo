...
function BHClient:tableFromString2 (str)
	-- Convert Python-style brackets to Lua-style table brackets
	--print(str)
    local lua_string = str
        :gsub("%[", "{")  -- Replace [ with {
        :gsub("%]", "}")  -- Replace ] with }
        :gsub("(%d) (%d)", "%1, %2") -- Add commas between numbers
		:gsub("%s+", " ") -- Replace multiple spaces with a single space
        :gsub("{ ", "{") -- Remove space after opening brace
        :gsub(" }", "}") -- Remove space before closing brace
	--print(lua_string)
    -- Load the string as a Lua table
    local lua_table = load("return " .. lua_string)()
    return lua_table
end

--[[ Converts a table to string-representation of the table
	 output format: "key:val,key:val,key:val" ]]
function BHClient:strigFromTable (tbl)
    local strng = ""
    
    for key, val in pairs(tbl) do
        strng = strng .. "," .. key .. ":" .. tostring(val)
    end
...
function BHClient:get (response)
	...
	local dataType, val = string.match(response, "([%w%[%]+]+)%s+(.+)")
...
--[[ Returns statement for getControls()
     Server's response should be passed to getControls() ]]
function BHClient:getControlsStatement()
    return "SET controls" .. self:strigFromTable(joypad.get())
end
...
function BHClient:read_classes(filename)
    ...
    -- Read all lines and trim whitespace
    local class_names = {}
    for line in file:lines() do
        local trimmed_line = line:match("^%s*(.-)%s*$") -- Trim spaces
        table.insert(class_names, trimmed_line)
    end
